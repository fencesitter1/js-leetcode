...
问总共有多少条不同的路径？
### 思路
主要思路是创建一个二维数组 dp，其中 dp[i][j]表示从起始点到达网格的(i, j)位置的不同路径的数量。对于每个位置，我们可以从它的上方或左方到达，所以 `dp[i][j]`就等于 `dp[i-1][j]`和 `dp[i][j-1]`的和。
...
```
- 优化版
在这个问题中，我们实际上并不需要一个完整的二维数组来存储所有的路径数量。因为在计算 dp[i][j]时，我们只需要知道 `dp[i-1][j]`和 `dp[i][j-1]`的值，也就是说，我们只需要知道当前行和上一行的路径数量。所以，我们可以使用一个一维数组来代替二维数组，以节省空间。
...
在这段代码中，`dp[j]` 存储的是当前行的路径数量，`dp[j - 1]` 存储的是上一行的路径数量。每次迭代时，我们更新 `dp[j]` 的值，使其等于当前行和上一行的路径数量之和。最后，返回 `dp[n - 1]`，即为总的不同路径的数量。
...
### Thinking
来自[笨猪爆破组](https://leetcode.cn/problems/unique-paths-ii/solutions/317379/shou-hua-tu-jie-dp-si-lu-by-hyj8/)
- 基本思路
![image.png|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F26%2F20240126174239_17-42-40.png)
- 障碍怎么处理?
![image.png|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F26%2F20240126174249_17-42-50.png)
...
作者：笨猪爆破组
链接：https://leetcode.cn/problems/unique-paths-ii/solutions/317379/shou-hua-tu-jie-dp-si-lu-by-hyj8/
来源：力扣（LeetCode）
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
```
...
### 思路
#### 递归
![image.png|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F27%2F20240127223215_22-32-15.png)
从顶向下,很好理解,但需要加入记忆化减少计算量
#### 动态规划
...
`dp[i]` 初始化：n=0和 n=1没有意义，dp[2]=1，所以直接初始化 dp 全为1
从前向后遍历
`j<=i-j `或者 `j<=i/2` 是为了减少的一般的计算量,例如`5=1*4,2*3,3*2..`
从 `3*2` 开始的计算就是重复的,可以不算.

### 代码
- 递归
```js
const integerBreak = (n) => {
  // 创建一个长度为n+1的数组，用于存储已经计算过的结果
  const memo = new Array(n + 1);

  // 定义一个递归函数，用于计算数字n拆分后的最大乘积
  const dfs = (n) => {
    // 如果数字n的结果已经计算过，直接返回结果
    if (memo[n]) return memo[n];

    let res = 0;
    // 尝试将数字n拆分为两个数字i和n-i
    for (let i = 1; i <= n - 1; i++) {
      // 比较不拆分n-i（即i*(n-i)）和拆分n-i（即i*dfs(n-i)）后的乘积哪个更大
...
var integerBreak = function (n) {
  ...
  for (let i = 3; i <= n; i++) {
    // 尝试将数字i拆分为两个数字j和i-j
    for (let j = 1; j <= i - j; j++) {
      // 比较不拆分i-j（即j*(i-j)）和拆分i-j（即j*dp[i-j]）后的乘积哪个更大
...
![image.png|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F28%2F20240128102851_10-28-51.png)
输入：n = 3
输出：5
### 思路
#### 动态规划
思路来自于[笨猪爆破组](https://leetcode.cn/problems/unique-binary-search-trees/solutions/330990/shou-hua-tu-jie-san-chong-xie-fa-dp-di-gui-ji-yi-h/)
![image.png|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F28%2F20240128103125_10-31-25.png)
...
**状态转移方程:**
	![image.png|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F28%2F20240128103328_10-33-28.png)

**初始条件:**

当 n=0时，没有数字，只能形成一种 BSTBSTBST ：空树。

当 n=1时，只有一个数字，只能形成一种 BSTBSTBST ：单个节点。
...
作者：笨猪爆破组
链接：https://leetcode.cn/problems/unique-binary-search-trees/solutions/330990/shou-hua-tu-jie-san-chong-xie-fa-dp-di-gui-ji-yi-h/
来源：力扣（LeetCode）
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
```
...
### 问题描述
给定一组物品，每种物品都有自己的重量和价值，现在给定一个背包，它能承受的最大重量是已知的，问如何选择物品放入背包，使得背包中物品的总价值最大。
### 思路

**解题思路如下：**

- 定义状态：我们可以定义一个二维数组 `dp[i][j]`，表示在前 i 个物品中选择一些物品放入容量为 j 的背包，能得到的最大价值。

- 状态转移：当我们做出物品 i 的决策后，剩余的是前 i-1个物品的决策，可分为以下两种情况(选择放入背包或者不放入背包)。
	 - 如果我们选择放入背包，那么背包的容量将减少(状态变化)，但是价值将增加；`dp[i-1][j-w[i]] + v[i]`
	 - 如果我们选择不放入背包，那么背包的容量和价值都不会改变,是上一个状态。`dp[i-1][j]`
...
3. 初始化状态：我们可以将 `dp[0][j]`初始化为0，表示在前0个物品中选择一些物品放入背包，能得到的最大价值是0。

4. 计算结果：最后，`dp[n][j]` 就是我们的答案，其中 n 是物品的总数，j 是背包的容量。

#### 动态规划
![knapsack-problem.gif|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F29%2Fknapsack-problem_15-26-00.gif)
...
进一步思考，我们能否仅用一个数组实现空间优化呢？观察可知，每个状态都是由正上方或左上方的格子转移过来的。假设只有一个数组，当开始遍历第 i 行时，该数组存储的仍然是第 i-1行的状态。

- 如果采取正序遍历，那么遍历到 `dp[i][j]` 时，左上方 `dp[i-1,1]`~`dp[i-1,j-1]` 值可能已经被覆盖，此时就无法得到正确的状态转移结果。
- 如果采取倒序遍历，则不会发生覆盖问题，状态转移可以正确进行。

**0-1 背包的空间优化后的动态规划过程**:
![knapsack-dp-onedim.gif|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F29%2Fknapsack-dp-onedim_16-45-50.gif)

**个人理解**:
![44351c73bd5462acd3773535552c088.jpg|500](https://cdn.jsdelivr.net/gh/fencesitter1/pictures/img/2024%2F01%2F29%2F44351c73bd5462acd3773535552c088_16-40-08.jpg)
##### 代码
```js
/* 0-1 背包：状态压缩后的动态规划 */
...
考虑数组 `[1, 2, 5, 11]`。这个数组的总和是19，是一个奇数。因此，我们不能将这个数组分割成两个和相等的子集，因为总和的一半（19/2 = 9.5）不是整数。

再考虑一个总和为偶数的例子：`[1, 2, 5, 10]`。这个数组的总和是18，是一个偶数。但是，我们不能将这个数组分割成两个和相等的子集，因为没有任何子集的和能够等于总和的一半（18/2 = 9）。

- 总和为偶的两种情况

如果一个数组的总和是偶数，但是不能被分割成两个和相等的子集，那么这个数组必须包含一个数，它大于总和的一半。
如果所有的数都小于或等于总和的一半，那么我们总是可以找到一种方式来分割这个数组。
...
var canPartition = function (nums) {
  ...
  // 定义一个辅助函数，用来判断是否可以找到一些数，它们的和等于 sum/2
  const helper = (tar) => {
    // 初始化动态规划数组，dp[j]表示能否找到一些数，它们的和等于j
    ...
    // 如果dp[tar]等于tar，说明背包可以装下tar,即可以分割为两个和相等的子集
    return dp[tar] === tar;
  };

  // 如果总和是偶数，调用helper函数；否则，返回false
  return sum % 2 === 0 ? helper(tar) : false;
};
```

**主要思路**：首先计算数组的总和，如果总和是奇数，那么肯定不能分割成两个和相等的子集，返回 false；如果总和是偶数，那么问题就转化为能否从数组中找到一些数，它们的和等于总和的一半。这是一个典型的背包问题，可以使用动态规划来解决。

**关键转化**:给你一个非空数组，和为 sum，你能否找到一个子序列，和为 sum/2。
...
### 思路
**关键**:尽量让石头分成重量相同的两堆，相撞之后剩下的石头最小，这样就化解成01背包问题了。
- 为什么问题能转为分成重量尽量相同的两堆?具体参考[宫水三叶](https://leetcode.cn/problems/last-stone-weight-ii/solutions/818362/gong-shui-san-xie-xiang-jie-wei-he-neng-jgxik/)的思路:
...
这意味我们最终得到的结果，可以为原来 stonesstonesstones 数组中的数字添加 +/−+/-+/− 符号，所形成的「计算表达式」所表示。
...
**为 stonesstonesstones 中的每个数字添加 +/−+/-+/−，使得形成的「计算表达式」结果绝对值最小。**

> [!warning] reduce 的格式
> let stoneSum = stones.reduce((sum, cur) => ~~(sum + cur, 0))~~;
>
>let stoneSum = stones.reduce((sum, cur) => sum + cur, 0);
>
> **一定注意没有括号**
...
var lastStoneWeightII = function (stones) {
  ...
  let stoneSum = stones.reduce((sum, cur) => sum + cur, 0);
  // 目标重量是总重量的一半，向下取整
  let tar = parseInt(stoneSum / 2);
  // 石头的数量
  let len = stones.length;
  // 初始化动态规划数组，长度为目标重量+1，所有元素初始化为0
  const dp = new Array(tar + 1).fill(0);
  ...
  for (let i = 0; i <= len - 1; i++) {
    for (let j = tar; j >= 0; j--) {
      // 如果当前石头的重量小于等于当前的目标重量
      if (j >= stones[i]) {
        // 更新dp[j]为dp[j]和dp[j - stones[i]] + stones[i]中的较大值
        dp[j] = Math.max(dp[j], dp[j - stones[i]] + stones[i]);
      }
    }
  }
  // dp[tar]是最接近目标重量的石头堆的重量
  let stonePile1 = dp[tar];
  // 剩余的石头堆的重量是总重量减去dp[tar]
  let stonePile2 = stoneSum - dp[tar]; //向下取整导致第二堆多
  // 返回两堆石头的重量差
  return stonePile2 - stonePile1;
};
```
